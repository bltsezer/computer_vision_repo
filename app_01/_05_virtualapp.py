...
class PID:
    ...
    def draw(self, img, cVal):
        ...
        if self.axis == 0:
            ...
            cv2.line(img, (self.targetVal, cVal[1]), (cVal[0], cVal[1]), (0, 255, 255), 1, 0)
        else:
            cv2.line(img, (0, self.targetVal), (w, self.targetVal), (255, 0, 255), 1)
            cv2.line(img, (cVal[0], self.targetVal), (cVal[0], cVal[1]), (0, 255, 255), 1, 0)

        cv2.circle(img, tuple(cVal), 5, (250, 25, 25), cv2.FILLED)
...
def main():
    ...
    detector2 = HandDetector(detectionCon=0.8, maxHands=1)
    ...
    while True:
        ...
        if bboxs:
            lmList, bboxInfo = detector2.findPosition(img)
